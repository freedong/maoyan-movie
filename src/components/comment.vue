...
import star from './star/star'
	export default {
		...
		mounted:function () {
			// 跨域请求数据
			const commenturl = 'https://api.douban.com/v2/movie/subject/' + this.$route.params.id + '/reviews?apikey=0b2bdeda43b5688921839c8ecb20399b&start=0&count=20&client=something&udid=dddddddddddddddddddddd'
			this.$http.jsonp(commenturl).then(function (response) {
				this.guodu = false
		...
		methods: {
			...
			// showSummary: function (index) {
			// 	this.commentsMsg.push({contentShow:false})
			// 	this.commentsMsg.reviews[index].contentShow = !this.commentsMsg.reviews[index].contentShow
			// 	console.log("nihao")
			// },
			// showContent: function (ind) {
			// 	this.commentsMsg.reviews[ind].contentShow = true
